    ...
    useEffect(() => {
        ...
        unsubs.push(onSnapshot(collection(db, 'submissions'), snap => {
            ...
            setStats(prev => ({
                ...
                pendingSubmissions: docs.filter((d: any) => d.status === 'pending' || d.status === 'reviewing' || d.status === 'first_read' || d.status === 'under_review').length,
        ...
        // Authors
        unsubs.push(onSnapshot(collection(db, 'authors'), snap => {
            setStats(prev => ({ ...prev, totalAuthors: snap.docs.length }));
        }, () => { }));

        return () => unsubs.forEach(u => u());
    }, []);

    const nav = (tabId: string) => { if (onNavigate) onNavigate(tabId); };

    const metricCards = [
        { label: 'Total Submissions', value: stats.totalSubmissions, icon: FileText, color: 'text-cosmic-purple', bg: 'bg-cosmic-purple/10', tab: 'submissions' },
        { label: 'Pending Review', value: stats.pendingSubmissions, icon: Clock, color: 'text-starforge-gold', bg: 'bg-starforge-gold/10', tab: 'submissions' },
        { label: 'Accepted', value: stats.acceptedSubmissions, icon: CheckCircle, color: 'text-aurora-teal', bg: 'bg-aurora-teal/10', tab: 'submissions' },
        { label: 'Books in Catalogue', value: stats.totalBooks, icon: BookOpen, color: 'text-starforge-gold', bg: 'bg-starforge-gold/10', tab: 'catalogue' },
        { label: 'Registered Users', value: stats.totalUsers, icon: Users, color: 'text-cosmic-purple', bg: 'bg-cosmic-purple/10', tab: 'users' },
        { label: 'Reviews', value: stats.totalReviews, icon: Eye, color: 'text-aurora-teal', bg: 'bg-aurora-teal/10', tab: 'reviews' },
        { label: 'Messages', value: stats.totalMessages, icon: MessageSquare, color: 'text-text-secondary', bg: 'bg-surface-elevated', tab: 'community' },
        { label: 'Authors', value: stats.totalAuthors, icon: Users, color: 'text-cosmic-purple', bg: 'bg-cosmic-purple/10', tab: 'authors' },
    ];

    const quickActions = [
        { label: 'Review Submissions', desc: `${stats.pendingSubmissions} pending`, color: 'border-cosmic-purple/30 hover:border-cosmic-purple/50', icon: FileText, tab: 'submissions' },
        { label: 'Editorial Pipeline', desc: 'Active projects', color: 'border-starforge-gold/30 hover:border-starforge-gold/50', icon: Scissors, tab: 'editorial' },
        { label: 'Manage Catalogue', desc: `${stats.totalBooks} books`, color: 'border-aurora-teal/30 hover:border-aurora-teal/50', icon: BookOpen, tab: 'catalogue' },
        { label: 'View Royalties', desc: 'Revenue & earnings', color: 'border-text-muted/20 hover:border-text-muted/40', icon: DollarSign, tab: 'royalties' },
    ];
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {metricCards.map((card, i) => (
                    <motion.div key={card.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        ...
                        className="bg-surface border border-border rounded-sm p-5 group hover:border-starforge-gold/20 transition-colors cursor-pointer">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Submission Queue */}
                <div className="bg-surface border border-border rounded-sm cursor-pointer hover:border-starforge-gold/20 transition-colors" onClick={() => nav('submissions')}>
                ...
                {/* Recent Users */}
                <div className="bg-surface border border-border rounded-sm cursor-pointer hover:border-starforge-gold/20 transition-colors" onClick={() => nav('users')}>
                    <div className="p-4 border-b border-border flex items-center justify-between">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {quickActions.map(action => (
                    <div key={action.label} onClick={() => nav(action.tab)}
                        className={`bg-surface border ${action.color} rounded-sm p-4 cursor-pointer transition-colors`}>
