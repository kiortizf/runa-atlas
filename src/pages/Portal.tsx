...
import { BookOpen, FileText, DollarSign, Settings, MessageSquare, Bell, LogOut, User, Star, X, Send, ExternalLink, Clock, CheckCircle, AlertCircle, Circle, ChevronRight, Loader2, Save, Folder, Download, Globe, Twitter, Instagram, BookMarked, LayoutDashboard, Mail, Lock, Eye, EyeOff, ArrowRight } from 'lucide-react';
...
// ─── Auth Gate Component ────────────────────────────
function PortalAuthGate({ signIn, signInWithEmail, signUpWithEmail, resetPassword, navigate }: {
  signIn: () => Promise<void>;
    ...
    <div className="min-h-screen bg-void-black flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <h1 className="font-display text-3xl text-white uppercase tracking-widest mb-2">Rüna Atlas</h1>
          <p className="font-ui text-sm text-text-secondary">Your gateway to speculative worlds</p>
        </div>

        {/* Card */}
        <div className="bg-white/[0.02] border border-white/[0.08] rounded-xl p-8 shadow-2xl">
          {/* Tabs */}
          <div className="flex mb-6 bg-white/[0.04] rounded-lg p-1">
            <button onClick={() => { setMode('signin'); setError(''); }}
              className={`flex-1 py-2.5 text-sm font-semibold rounded-md transition-all ${mode === 'signin' ? 'bg-starforge-gold text-void-black' : 'text-white/50 hover:text-white/70'}`}>
              Sign In
            </button>
            <button onClick={() => { setMode('signup'); setError(''); }}
              className={`flex-1 py-2.5 text-sm font-semibold rounded-md transition-all ${mode === 'signup' ? 'bg-starforge-gold text-void-black' : 'text-white/50 hover:text-white/70'}`}>
              Create Account
            </button>
          </div>
          ...
          {/* Error */}
          {error && (
            <div className="mb-4 p-3 bg-forge-red/10 border border-forge-red/20 rounded-lg">
              <p className="text-xs text-forge-red">{error}</p>
            </div>
          )}
            ...
            <form onSubmit={handleEmailSignIn} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                    ...
                    className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg pl-10 pr-4 py-3 text-white text-sm placeholder:text-white/20 focus:border-starforge-gold/40 focus:outline-none transition-colors" />
                </div>
              </div>
              <div>
                <label className="text-[10px] text-white/40 uppercase tracking-wider block mb-1.5">Password</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20" />
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required
                    placeholder="••••••••"
                    className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg pl-10 pr-10 py-3 text-white text-sm placeholder:text-white/20 focus:border-starforge-gold/40 focus:outline-none transition-colors" />
              ...
              <div className="flex justify-end">
                <button type="button" onClick={() => { setShowReset(true); setResetEmail(email); setResetSent(false); }}
                  className="text-[11px] text-starforge-gold hover:text-yellow-400 transition-colors">
                  Forgot password?
                </button>
              </div>
              <button type="submit" disabled={loading}
                className="w-full py-3 bg-starforge-gold text-void-black text-sm font-semibold rounded-lg hover:bg-yellow-400 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          {mode === 'signup' && (
            <form onSubmit={handleEmailSignUp} className="space-y-4">
              <div>
                <label className="text-[10px] text-white/40 uppercase tracking-wider block mb-1.5">Full Name</label>
                <div className="relative">
              ...
              <div>
                <label className="text-[10px] text-white/40 uppercase tracking-wider block mb-1.5">Confirm Password</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20" />
          ...
          <button onClick={handleGoogleSignIn}
            className="w-full py-3 bg-white/[0.04] border border-white/[0.1] text-white text-sm font-medium rounded-lg hover:bg-white/[0.08] transition-colors flex items-center justify-center gap-3">
      ...
      {showReset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowReset(false)}>
          ...
          <div className="relative bg-deep-space border border-border rounded-xl p-8 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <button onClick={() => setShowReset(false)} className="absolute top-4 right-4 text-white/30 hover:text-white/60">
              <X className="w-4 h-4" />
            </button>
            <h3 className="font-display text-xl text-white uppercase tracking-wider mb-2">Reset Password</h3>
            {resetSent ? (
              <div className="text-center py-4">
                <CheckCircle className="w-10 h-10 text-aurora-teal mx-auto mb-3" />
                <p className="text-sm text-text-secondary">Check your email for a reset link.</p>
              </div>
            ) : (
              <>
                ...
                <input type="email" value={resetEmail} onChange={e => setResetEmail(e.target.value)}
                  placeholder="you@example.com"
                  className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg px-4 py-3 text-white text-sm placeholder:text-white/20 focus:border-starforge-gold/40 focus:outline-none mb-4" />
                <button onClick={handleResetPassword}
                  className="w-full py-3 bg-starforge-gold text-void-black text-sm font-semibold rounded-lg hover:bg-yellow-400 transition-colors">
                  Send Reset Link
                </button>
              </>
            )}
...
export default function Portal() {
  ...
  const { user, userRole, isAuthReady, signIn, signInWithEmail, signUpWithEmail, resetPassword, logOut } = useAuth();
  ...
  if (!user) {
    return <PortalAuthGate signIn={signIn} signInWithEmail={signInWithEmail} signUpWithEmail={signUpWithEmail} resetPassword={resetPassword} navigate={navigate} />;
  }
